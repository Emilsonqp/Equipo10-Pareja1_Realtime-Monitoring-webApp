...
def get_top_countries(request, **kwargs):
    data_result = {}

    top = request.GET.get("n", None)
    measurements = Measurement.objects.all()
    ...
    for measurement in measurements:
        ...
        for location in locations:            
            stations = Station.objects.filter(location=location)
            locationData = Data.objects.filter(
                station__in=stations, measurement__name=measurement.name,  time__gte=start.date(), time__lte=end.date())            
            if locationData.count() <= 0:
                continue
        ...
        for k in data:
            ...
            if c > int(top):
                data.remove(k)
        data_result[measurement.name] = data
